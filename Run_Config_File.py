...
#        return classifier_dict['NODE_0'].Predicting(row.reshape(-1,1))  
    ...
    def Build_Classifiers(self):
        ...
        print 'Test'
        self.data_process = Data_Processing(config.section_dict['DATA']['fft'],config.section_dict['DATA']['detrend'],config.section_dict['DATA']['norm'])
        
	seismic_loc = os.path.join(os.getcwd(),config.section_dict['DATA']['seismic_data'])
        explosion_loc = os.path.join(os.getcwd(),config.section_dict['DATA']['explosion_data'])
	nuclear_loc = os.path.join(os.getcwd(),config.section_dict['DATA']['nuclear_data'])
            ...
            if 'NODE' in key:
                ...
                if config.section_dict['DATA']['validate_size'] != str(0):
                    ...
                    node_name = Classifier(config.section_dict[key]['classifier'],eval(config.section_dict[key]['num_iter']),
                                           eval(config.section_dict[key]['optimization']))
